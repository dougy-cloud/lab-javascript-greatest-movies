...
// and filter 👀 the array so we can have 
// only the drama movies 
// where Steven Spielberg is the director.



function howManyMovies(moviesArray) {
    const dramaMovies = moviesArray.filter(drama => drama.director === "Steven Spielberg" && drama.genre.includes('Drama'));
...
/*       const total = moviesArray.reduce((sum, movie) => {
    if (movie.score) {
      ...
      let savedScores = moviesArray.map(scr => scr.score);
    let total = savedScores.reduce((current, next) => { 
        return current + scr.score
    });

    if (moviesArray.length > 0){
    average = total/savedScores.length;
    }
...
function dramaMoviesScore(moviesArray) {
    ...
    totalSum = dramaMovies.reduce((accumulator, next) =>{ return accumulator + next.score}, 0)
    result = totalSum/dramaMovies.length
    }
    else {
        result = 0
    }
...
// for the 5 you just need to do 2 compare function, 
// one to order the title first 
// and the other to order the year after, then just make a new array just to clear the modify array (dont know why they put it)

/* WHAT'S WRONG - WHAT IS MISSING - FOR LATER
function orderByYear(moviesArray) {
    const copyMovies = [...moviesArray].sort(movies => {
        if (movies.year){
            return movies;
        }
...
// the 6 also put a compare function to then sort by title, make a new array with just the titles with a for loop for the limit of the first 20 elements
...
function orderAlphabetically(moviesArray) {
    const firstTwenty = [...moviesArray].slice(0, 20);
    const sortedTwenty = firstTwenty.sort((a, b) => a.title - b.title);
    return sortedTwenty;
}
*/
